// ═══════════════════════════════════════════════════════════════════════════
// FamilyForge - EMAIL SYSTEM PRO (PRODUCTION)
// ═══════════════════════════════════════════════════════════════════════════
//
// ⚠️  CRITICAL WARNING: DO NOT EDIT THIS FILE
//
// This is the PRODUCTION email system. All templates are mobile-optimized
// using table-based layouts with bgcolor attributes for maximum email client
// compatibility (iOS Mail, Gmail, Outlook, etc.)
//
// READ WARNING.md BEFORE MAKING ANY CHANGES
//
// Key Architecture:
// - 11 premium email templates (welcome, verification, reports, etc.)
// - Mobile-safe: <table> + bgcolor (NOT <div> + CSS background)
...
const emailHeader = (title: string, subtitle: string = '') => `
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-radius: 24px 24px 0 0; overflow: hidden;">
    <tr>
      <td style="background: linear-gradient(135deg, ${BRAND.primary} 0%, ${BRAND.secondary} 50%, #312e81 100%); padding: 3px 3px 0 3px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr>
            <td bgcolor="#1a1033" style="background-color: #1a1033; border-radius: 22px 22px 0 0; padding: 48px 32px 40px; text-align: center;">
              <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td align="center">
                    <table cellpadding="0" cellspacing="0" border="0" style="border: 1px solid rgba(255,255,255,0.1); border-radius: 20px;">
                      <tr>
                        <td bgcolor="#2a1f4e" style="background-color: #2a1f4e; border-radius: 20px; padding: 0;">
                ...
                <tr>
                  <td align="center" style="padding-top: 20px;">
                    <h1 style="color: #ffffff; margin: 0; font-size: 28px; font-weight: 800; letter-spacing: -0.5px; mso-line-height-rule: exactly;">FamilyForge</h1>
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td align="center" style="padding-top: 32px;">
                    <table width="80%" cellpadding="0" cellspacing="0" border="0">
                      <tr><td style="border-top: 1px solid rgba(139, 92, 246, 0.2); padding-top: 24px; text-align: center;">
                        <h2 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700; line-height: 1.3;">${title}</h2>
...
const emailFooter = () => `
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-radius: 0 0 24px 24px; overflow: hidden;">
    <tr>
      <td style="background: linear-gradient(135deg, ${BRAND.primary} 0%, ${BRAND.secondary} 50%, #312e81 100%); padding: 0 3px 3px 3px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr>
            <td bgcolor="${BRAND.dark}" style="background-color: ${BRAND.dark}; border-radius: 0 0 22px 22px; padding: 40px 32px; text-align: center;">
              <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td align="center" style="padding-bottom: 24px;">
                    <table cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${BRAND.primary}" style="background-color: ${BRAND.primary}; width: 8px; height: 8px; border-radius: 50; font-size: 1px; line-height: 1px;">&nbsp;</td></tr></table>
                  </td>
                </tr>
                <tr>
                  <td align="center">
                ...
                <tr>
                  <td align="center" style="padding: 16px 0;">
                    <a href="https://familyforge.app" style="color: ${BRAND.text}; text-decoration: none; font-size: 13px; font-weight: 500; padding: 8px 16px; margin: 4px; border-radius: 20px; border: 1px solid rgba(139, 92, 246, 0.2); display: inline-block;">Website</a>
                    <a href="https://familyforge.app/support" style="color: ${BRAND.text}; text-decoration: none; font-size: 13px; font-weight: 500; padding: 8px 16px; margin: 4px; border-radius: 20px; border: 1px solid rgba(139, 92, 246, 0.2); display: inline-block;">Support</a>
                ...
                <tr>
                  <td align="center" style="padding-top: 20px;">
                    <p style="color: #475569; font-size: 11px; margin: 0; letter-spacing: 0.5px;">
                      &copy; ${new Date().getFullYear()} FamilyForge Inc. All rights reserved.
                    </p>
                  </td>
                </tr>
              </table>
...
// Mobile-safe body wrapper (table + bgcolor for iOS Mail)
const emailBodyOpen = () => `<!DOCTYPE html>
<html>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#080510" style="background-color: #080510;"><tr><td align="center" style="padding: 20px;">
<table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; width: 100%;"><tr><td>`;

const emailBodyEnd = () => `</td></tr></table>
</td></tr></table>
</body></html>`;

// Content section wrapper with solid border + bgcolor fallback
const contentWrapOpen = (center = false) => `
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-left: 3px solid ${BRAND.primary}; border-right: 3px solid ${BRAND.primary};">
  <tr><td bgcolor="${BRAND.dark}" style="background-color: ${BRAND.dark}; padding: 40px 32px;${center ? ' text-align: center;' : ''}">`;

const contentWrapClose = () => `
  </td></tr>
</table>`;
...
const emailTemplates: Record<string, (data: Record<string, unknown>) => { subject: string; html: string }> = {
  ...
  email_verification_code: (data) => ({
    subject: `${data.parentName || 'Hey'}, your FamilyForge verification code is ${data.code}`,
    html: `
      ${emailBodyOpen()}

          ${emailHeader('Verify Your Email', "You're one step away from getting started")}
          ...
          ${contentWrapOpen()}
              ...
              <table cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                  <td width="48" style="vertical-align: top;">
                    <div style="width: 48px; height: 48px; background: linear-gradient(135deg, ${BRAND.primary}, ${BRAND.secondary}); border-radius: 50%; text-align: center; line-height: 48px;">
                      <span style="color: white; font-weight: 700; font-size: 18px;">${String(data.parentName || 'U').charAt(0).toUpperCase()}</span>
                    </div>
                  </td>
                  <td style="padding-left: 16px; vertical-align: top;">
                    <p style="color: white; font-size: 20px; font-weight: 600; margin: 0 0 4px;">Almost there, ${data.parentName || 'there'}!</p>
                    <p style="color: ${BRAND.muted}; font-size: 14px; margin: 0;">Let's confirm it's really you</p>
                  </td>
                </tr>
              </table>

              <p style="color: ${BRAND.text}; line-height: 1.8; margin: 28px 0 24px; font-size: 15px;">
                We're excited to have you on board! To keep your family's account safe and secure, please enter this 4-digit verification code in the app.
              </p>

              <!-- Code display card — premium style like invite code -->
              <div style="background: linear-gradient(145deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%); border: 1px solid rgba(255,255,255,0.1); padding: 32px; border-radius: 16px; margin: 24px 0; text-align: center;">
                <div style="width: 56px; height: 56px; background: linear-gradient(135deg, rgba(16, 185, 129, 0.2) 0%, rgba(16, 185, 129, 0.05) 100%); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 16px; margin: 0 auto 20px; text-align: center; line-height: 56px;">
                  <span style="font-size: 28px;">&#128274;</span>
                </div>
                <p style="color: ${BRAND.muted}; margin: 0 0 12px; font-size: 12px; text-transform: uppercase; letter-spacing: 2px; font-weight: 600;">Your Verification Code</p>
                <p style="color: white; font-size: 44px; font-weight: 800; letter-spacing: 12px; margin: 0; font-family: 'Courier New', Courier, monospace;">${data.code}</p>
              </div>

              <!-- Expiry warning -->
              <div style="background: linear-gradient(135deg, rgba(251, 191, 36, 0.1) 0%, rgba(245, 158, 11, 0.05) 100%); border: 1px solid rgba(251, 191, 36, 0.25); padding: 16px 20px; border-radius: 12px; margin: 24px 0; text-align: center;">
                <p style="color: ${BRAND.gold}; margin: 0; font-size: 14px; font-weight: 600;">
                  &#9203; This code expires in 10 minutes
                </p>
              </div>

              <!-- How it works steps -->
              <p style="color: white; font-size: 16px; font-weight: 700; margin: 32px 0 16px;">How to verify:</p>
              ...
              <div style="background: linear-gradient(145deg, rgba(16, 185, 129, 0.1) 0%, rgba(16, 185, 129, 0.02) 100%); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: 16px; padding: 20px; margin-bottom: 12px;">
                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                  <tr>
                    ...
                    <td style="padding-left: 16px; vertical-align: top;">
                      <p style="color: white; margin: 0 0 4px; font-weight: 600;">Open FamilyForge on your device</p>
                      <p style="color: ${BRAND.muted}; margin: 0; font-size: 13px;">You should be on the verification screen</p>
                    </td>
                  </tr>
                </table>
              </div>
              ...
              <div style="background: linear-gradient(145deg, rgba(139, 92, 246, 0.1) 0%, rgba(139, 92, 246, 0.02) 100%); border: 1px solid rgba(139, 92, 246, 0.2); border-radius: 16px; padding: 20px; margin-bottom: 12px;">
                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                  <tr>
                    ...
                    <td style="padding-left: 16px; vertical-align: top;">
                      <p style="color: white; margin: 0 0 4px; font-weight: 600;">Enter the 4-digit code above</p>
                      <p style="color: ${BRAND.muted}; margin: 0; font-size: 13px;">Type it carefully — codes are case-sensitive</p>
                    </td>
                  </tr>
                </table>
              </div>
              ...
              <div style="background: linear-gradient(145deg, rgba(251, 191, 36, 0.1) 0%, rgba(251, 191, 36, 0.02) 100%); border: 1px solid rgba(251, 191, 36, 0.2); border-radius: 16px; padding: 20px; margin-bottom: 32px;">
                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                  <tr>
                    ...
                    <td style="padding-left: 16px; vertical-align: top;">
                      <p style="color: white; margin: 0 0 4px; font-weight: 600;">You're all set!</p>
                      <p style="color: ${BRAND.muted}; margin: 0; font-size: 13px;">Your email is confirmed and your account is secured</p>
                    </td>
                  </tr>
                </table>
              </div>
              ...
              <div style="background: linear-gradient(135deg, rgba(139, 92, 246, 0.15) 0%, rgba(79, 70, 229, 0.1) 100%); border-left: 4px solid ${BRAND.primary}; padding: 20px 24px; border-radius: 0 16px 16px 0; margin: 32px 0;">
                <p style="color: ${BRAND.accent}; font-size: 15px; font-style: italic; margin: 0; line-height: 1.6;">
                  "Your family's transformation starts with this one small step. We can't wait to see what you'll accomplish together."
                </p>
              </div>

              <!-- Security note -->
              <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 12px; padding: 16px 20px; margin-top: 24px;">
                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                  <tr>
                    ...
                    <td style="padding-left: 8px; vertical-align: top;">
                      <p style="color: ${BRAND.muted}; font-size: 13px; margin: 0; line-height: 1.6;">
                        <strong style="color: ${BRAND.text};">Security note:</strong> We'll never ask for this code via phone or social media. If you didn't create a FamilyForge account, you can safely ignore this email.
                      </p>
                    </td>
                  </tr>
                </table>
